...
#define I2C_ENDTRANS()                      _i2c->endTransmission(true)
#define I2C_REQUESTFROM(addr, bytes, stop)  _i2c->requestFrom(addr, bytes, stop)
...
#define I2C_ENDTRANS()                      _i2c->endTransmission()
#define I2C_REQUESTFROM(addr, bytes, stop)  _i2c->requestFrom(addr, bytes)

AHTxx::AHTxx(uint8_t address, AHTXX_I2C_SENSOR sensorType, I2CBusBase &i2c) : _i2c(&i2c)
{
  _address    = address;
  _sensorType = sensorType;
  _status     = AHTXX_NO_ERROR;
...
bool AHTxx::begin(uint32_t speed, uint32_t stretch)
{
  ...
  _i2c->setClock(speed);                                    //experimental! AVR I2C bus speed 31kHz..400kHz, default 100000Hz

  #if !defined (__AVR_ATtiny85__)                          //for backwards compatibility with ATtiny Core
  _i2c->setWireTimeout(stretch, false);                     //experimental! default 25000usec, true=Wire hardware will be automatically reset to default on timeout
  #endif
...
bool AHTxx::begin(uint8_t sda, uint8_t scl, uint32_t speed, uint32_t stretch)
{
  ...
  _i2c->setClock(speed);                                    //experimental! ESP8266 I2C bus speed 1kHz..400kHz, default 100000Hz

  _i2c->setClockStretchLimit(stretch);                      //experimental! default 150000usec
...
{
  if (_i2c->begin(sda, scl, speed) != true) {return false;} //experimental! ESP32 I2C bus speed ???kHz..400kHz, default 100000Hz

  _i2c->setTimeout(stretch / 1000);                         //experimental! default 50msec
...
bool AHTxx::begin(uint32_t sda, uint32_t scl, uint32_t speed) //"uint32_t" for pins only, "uint8_t" calls wrong "setSCL(PinName scl)"
{
  ...
  _i2c->setClock(speed);                                    //experimental! STM32 I2C bus speed ???kHz..400kHz, default 100000Hz
...
bool LiquidCrystal_I2C::begin(uint8_t columns, uint8_t rows, lcdFontSize fontSize, uint32_t speed)
{
  ...
  _i2c->setClock(speed);                                    //experimental! SAMD21 I2C bus speed ???kHz..400kHz, default 100000Hz
...
bool AHTxx::softReset()
{
  ...
  _i2c->write(AHTXX_SOFT_RESET_REG);
...
void AHTxx::_readMeasurement()
{
  ...
  _i2c->write(AHTXX_START_MEASUREMENT_REG);      //send measurement command, strat measurement
  _i2c->write(AHTXX_START_MEASUREMENT_CTRL);     //send measurement control
  _i2c->write(AHTXX_START_MEASUREMENT_CTRL_NOP); //send measurement NOP control
  ...
  I2C_REQUESTFROM(_address, dataSize, (uint8_t)true); //read n-byte to "wire.h" rxBuffer, true-send stop after transmission

  if (_i2c->available() != dataSize)
  {
  ...
  //_i2c->readBytes(_rawData, dataSize);                  //"readBytes()", from Stream Class 
  // replacement for readBytes()
  uint8_t ndx = 0;
  while(dataSize--) {
    _rawData[ndx] = (uint8_t)_i2c->read();
    if(++ndx > ArraySize(_rawData))
      break;
  }
...
bool AHTxx::_setInitializationRegister(uint8_t value)
{
  ...
  if   (_sensorType == AHT1x_SENSOR) {_i2c->write(AHT1X_INIT_REG);} //send initialization command, for AHT1x only
  else                               {_i2c->write(AHT2X_INIT_REG);} //send initialization command, for AHT2x only

  _i2c->write(value);                                               //send initialization register controls
  _i2c->write(AHTXX_INIT_CTRL_NOP);                                 //send initialization register NOP control
...
uint8_t AHTxx::_readStatusRegister()
{
  ...
  _i2c->write(AHTXX_STATUS_REG);

  I2C_ENDTRANS();

  I2C_REQUESTFROM(_address, (uint8_t)1, (uint8_t)true);     //read 1-byte to "wire.h" rxBuffer, true-send stop after transmission

  if (_i2c->available() == 1) {return _i2c->read();}           //read 1-byte from "wire.h" rxBuffer
...
uint8_t AHTxx::_getBusy(bool readAHT)
{
  if (readAHT == AHTXX_FORCE_READ_DATA)                    //force to read data via I2C & update "_rawData[]" buffer
  {
    ...
    if (_i2c->available() != 1) {return AHTXX_DATA_ERROR;}  //no reason to continue, "return" terminates the entire function & "break" just exits the loop

    _rawData[0] = _i2c->read();                             //read 1-byte from "wire.h" rxBuffer
  }
